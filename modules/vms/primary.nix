...
in {
  ...
  in {
    ...
    in [
      "hugepages=${vm.ramGb.vm}" "nohz_full=${vm.vmCores.list}" "rcu_nocbs=${vm.vmCores.list}"
    ...
    systemd.slices.nix-idle.sliceConfig.AllowedCPUs = vm.vmCores.list;
    services.vfio-irq-balance.affinityMask = vm.vmCores.mask;
  });
}